...
interface ClienteSelect { id_cliente: number; nome: string; }
interface EspacoSelect { id_espaco: number; nome: string; preco: number; diasIndisponiveis: string[]; }
...
export default function ReservaForm({ reserva, onClose, onSave }: ReservaFormProps) {
    ...
    const [diasLocados, setDiasLocados] = useState<string[]>([]);
    ...
    useEffect(() => {
        const selectedEspaco = espacosSelect.find(
            (e) => e.id_espaco === Number(formData.id_espaco)
        );
        if (selectedEspaco) {
            console.log("Espaço selecionado:", selectedEspaco);
            setFormData(prev => ({
                ...prev,
                valorTotal: selectedEspaco.preco.toFixed(2),
                valorPago: isEditing ? prev.valorPago : selectedEspaco.preco.toFixed(2),
            }));
            setDiasLocados(selectedEspaco.diasIndisponiveis ?? []);
        } else {
            setDiasLocados([]);
        }
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        console.log(`Valor do campo ${name} alterado para: ${value}`);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        console.log("Dados da reserva a salvar:", dataToSave);
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex justify-center items-center">
            <div className="relative bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="id_espaco" id="id_espaco" value={formData.id_espaco} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                {espacosSelect.map(e => (
                                    <option key={e.id_espaco} value={e.id_espaco}>{e.nome} (R$ {e.preco.toFixed(2)})</option>
                                ))}
                            </select>
                        </div>
                    ...
                    {diasLocados.length > 0 && (
                        <div className="p-3 bg-yellow-50 border-l-4 border-yellow-500 text-sm text-yellow-800 rounded-md">
                            ⚠️ Datas já Locadas para este Espaço: 
